...
class DriverViewSet(viewsets.ModelViewSet):
    ...
    def update_location(self, request, pk=None):
        ...
        if driver.user != request.user:
            return Response(
                {"error": "No tienes permisos para actualizar esta ubicación"}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def update_availability(self, request, pk=None):
        ...
        if driver.user != request.user:
            return Response(
                {"error": "No tienes permisos para cambiar esta disponibilidad"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        return Response({"message": "Disponibilidad actualizada", "availability": driver.availability})
    ...
    def active_trips(self, request, pk=None):
        ...
        trips = Trip.objects.filter(driver=driver, status__in=["accepted", "driver_arrived", "in_progress"]).order_by(
            "-created_at"
        )
    ...
    def earnings(self, request, pk=None):
        ...
        trips = Trip.objects.filter(driver=driver, status="completed")
...
class TripViewSet(viewsets.ModelViewSet):
    ...
    def accept_trip(self, request, pk=None):
        ...
        if not hasattr(request.user, "driver_profile"):
            return Response({"error": "Solo conductores pueden aceptar viajes"}, status=status.HTTP_403_FORBIDDEN)

        driver = request.user.driver_profile

        if trip.status != "requested":
            return Response({"error": "Este viaje ya no está disponible"}, status=status.HTTP_400_BAD_REQUEST)

        if driver.availability != "available":
            return Response({"error": "No estás disponible para aceptar viajes"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"message": "Viaje aceptado correctamente", "trip": TripSerializer(trip).data})
    ...
    def start_trip(self, request, pk=None):
        ...
        if trip.driver.user != request.user:
            return Response({"error": "No eres el conductor asignado"}, status=status.HTTP_403_FORBIDDEN)

        if trip.status not in ["accepted", "driver_arrived"]:
            return Response({"error": "No puedes iniciar este viaje"}, status=status.HTTP_400_BAD_REQUEST)

        trip.status = "in_progress"
        trip.started_at = timezone.now()
        trip.save()

        return Response({"message": "Viaje iniciado", "trip": TripSerializer(trip).data})
    ...
    def complete_trip(self, request, pk=None):
        ...
        if trip.status != "in_progress":
            return Response({"error": "El viaje debe estar en progreso"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"message": "Viaje completado", "trip": TripSerializer(trip).data})
    ...
    def cancel_trip(self, request, pk=None):
        ...
        if trip.customer.user != request.user and (trip.driver and trip.driver.user != request.user):
            return Response({"error": "No tienes permisos para cancelar este viaje"}, status=status.HTTP_403_FORBIDDEN)
        ...
        return Response({"message": "Viaje cancelado", "trip": TripSerializer(trip).data})
...
class TripRatingViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "customer" and hasattr(user, "customer_profile"):
            return TripRating.objects.filter(trip__customer=user.customer_profile, rating_type="customer_to_driver")
        elif hasattr(user, "driver_profile"):
            return TripRating.objects.filter(trip__driver=user.driver_profile, rating_type="driver_to_customer")
        else:
...
def trip_estimate(request):
    ...
    if not all([pickup_lat, pickup_lon, dest_lat, dest_lon]):
        return Response({"error": "Se requieren coordenadas de origen y destino"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response(
            {
                ...
                "pricing": pricing,
                "available_drivers": Driver.objects.filter(status="approved", availability="available").count(),
            }
        )

    except Exception as e:
        return Response(
            {"error": f"Error calculando estimación: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            name="latitude", description="Latitud de la ubicación", required=True, type=OpenApiTypes.FLOAT
        ),
        OpenApiParameter(
            name="longitude", description="Longitud de la ubicación", required=True, type=OpenApiTypes.FLOAT
        ),
        OpenApiParameter(
            name="radius",
...
def nearby_drivers(request):
    ...
    if not lat or not lon:
        return Response({"error": "Se requieren parámetros latitude y longitude"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        for driver in available_drivers:
            distance = calculate_distance(lat, lon, driver.current_latitude, driver.current_longitude)

            if distance <= radius:
                nearby_drivers.append({"driver": DriverSerializer(driver).data, "distance": round(distance, 2)})

        # Ordenar por distancia
        nearby_drivers.sort(key=lambda x: x["distance"])

        return Response({"drivers": nearby_drivers, "total_count": len(nearby_drivers)})

    except Exception as e:
        return Response(
            {"error": f"Error buscando conductores: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
