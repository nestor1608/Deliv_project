...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for nt_data in notification_types:
            notification_type, created = NotificationType.objects.get_or_create(name=nt_data["name"], defaults=nt_data)

            if created:
                self.stdout.write(self.style.SUCCESS(f"Creado tipo de notificación: {notification_type.name}"))
            else:
