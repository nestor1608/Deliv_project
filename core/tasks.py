...
def cleanup_old_notifications():
    ...
    deleted_count = Notification.objects.filter(created_at__lt=cutoff_date, status__in=["delivered", "read"]).delete()[
        0
    ]
...
def update_delivery_person_availability():
    ...
    updated = DeliveryPerson.objects.filter(last_location_update__lt=cutoff_time, availability="available").update(
