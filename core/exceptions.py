...
def handler404(request, exception):
    ...
    return JsonResponse({"error": "page_not_found", "message": "Página no encontrada"}, status=404)
...
def handler500(request):
    ...
    return JsonResponse({"error": "server_error", "message": "Error del servidor"}, status=500)
...
def handler403(request, exception):
    ...
    return JsonResponse({"error": "access_denied", "message": "Acceso denegado"}, status=403)
...
def validate_phone_number(value):
    ...
    if not phone_regex.match(value):
        raise ValidationError("Formato de teléfono inválido. Debe incluir código de país y tener entre 9 y 15 dígitos.")
