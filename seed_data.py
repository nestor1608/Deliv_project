...
def create_users():
    ...
    if not User.objects.filter(email='admin@deliv.com').exists():
        User.objects.create_superuser(f'admin_{suffix}', 'admin@deliv.com', 'password123', role='admin', phone_number=f'+{suffix}999999')

    # Customer
    customer_user, _ = User.objects.get_or_create(
        email=f'customer_{suffix}@deliv.com',
        defaults={'username':f'customer_{suffix}', 'first_name': 'Carlos', 'last_name': 'Cliente', 'role': 'customer', 'phone_number': f'+{suffix}111111'}
    )
    if _:
    ...
    vendor_user, _ = User.objects.get_or_create(
        ...
        defaults={'username':f'vendor_{suffix}', 'first_name': 'Victor', 'last_name': 'Vendedor', 'role': 'vendor', 'phone_number': f'+{suffix}222222'}
    )
    if _:
    ...
    delivery_user, _ = User.objects.get_or_create(
        ...
        defaults={'username':f'delivery_{suffix}', 'first_name': 'Diego', 'last_name': 'Delivery', 'role': 'delivery', 'phone_number': f'+{suffix}333333'}
    )
    if _:
...
def create_vendor_data(vendor_user):
    ...
    category, _ = VendorCategory.objects.get_or_create(
        name='Restaurantes',
        defaults={'description': 'Comida preparada'}
    )
