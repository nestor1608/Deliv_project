...
from .serializers import (
    ...
    RefundSerializer, RefundCreateSerializer, ProcessPaymentSerializer
)
...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def refund(self, request, pk=None):
        ...
        if payment.refunds.filter(status__in=['pending', 'processing']).exists():
            return Response(
                {'error': 'Ya hay un reembolso en proceso para este pago'},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def statistics(self, request):
        ...
        stats = {
            ...
            'total_amount': user_payments.filter(
                status='completed'
        ...
        # Pagos por método
        by_method = user_payments.values(
...
def process_payment(request):
    ...
    try:
        ...
        if object_type == 'order':
            ...
            return Response(
                {'error': 'Tipo de objeto no válido'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if existing_payment:
            return Response(
                {'error': 'Este objeto ya ha sido pagado'},
                status=status.HTTP_400_BAD_REQUEST
            )
...
def stripe_webhook(request):
    ...
    """
    # TODO: Implementar webhook de Stripe cuando se integre
    return HttpResponse(status=200)
...
@extend_schema(
    ...
    tags=['payments'],
    parameters=[
        OpenApiParameter(name='days', description='Número de días a analizar (default: 30)', required=False, type=OpenApiTypes.INT),
    ],
...
def payment_analytics(request):
    ...
    # Por método de pago
    by_method = payments.filter(status='completed').values(
        'payment_method__name'
    ).annotate(
        count=models.Count('id'),
        total_amount=Sum('amount')
    ...
    for i in range(7):
        ...
        daily_stats.append({
            ...
            'amount': day_payments.filter(status='completed').aggregate(
                total=Sum('amount')
            )['total'] or 0
        })
