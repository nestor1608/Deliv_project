...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                "status", OpenApiTypes.STR, description="Filtrar por estado (approved, pending, rejected)"
            ),
            OpenApiParameter("is_open", OpenApiTypes.BOOL, description="Filtrar por comercios abiertos/cerrados"),
            OpenApiParameter(
                "search", OpenApiTypes.STR, description="Buscar por nombre del comercio, descripción o categoría"
            ),
            OpenApiParameter(
                "ordering",
                OpenApiTypes.STR,
                description="Ordenar por rating, delivery_time, delivery_fee, created_at. Prefijo - para descendente.",
            ),
        ],
    ),
...
class VendorViewSet(viewsets.ModelViewSet):
    ...
    def products(self, request, pk=None):
        ...
        if product_search:
            products = products.filter(Q(name__icontains=product_search) | Q(description__icontains=product_search))
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                "latitude", OpenApiTypes.FLOAT, description="Latitud de la ubicación de referencia", required=True
            ),
            OpenApiParameter(
                "longitude", OpenApiTypes.FLOAT, description="Longitud de la ubicación de referencia", required=True
            ),
        ],
    ...
    def nearby(self, request):
        ...
        if not lat or not lng:
            return Response(
                {"error": "Se requieren parámetros latitude y longitude"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"error": "latitude y longitude deben ser valores numéricos"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def toggle_status(self, request, pk=None):
        ...
        return Response({"status": "abierto" if vendor.is_open else "cerrado", "is_open": vendor.is_open})
