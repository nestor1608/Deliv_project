...
class OrderListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return Order.objects.filter(customer=self.request.user.customer_profile).prefetch_related(
...
class VendorOrdersView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Order.objects.filter(vendor=self.request.user.vendor_profile).prefetch_related(
            "items__product", "status_history"
        )
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            "order_id", OpenApiTypes.INT, location=OpenApiParameter.PATH, description="ID del pedido a actualizar"
        ),
    ],
    responses={
        200: OrderSerializer,
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            ...
            description="Número único de pedido para seguimiento",
        ),
    ],
    responses={
...
def order_tracking(request, order_number):
    ...
    try:
        ...
        if request.user.role == "customer":
            order = Order.objects.get(order_number=order_number, customer=request.user.customer_profile)
        elif request.user.role == "vendor":
            order = Order.objects.get(order_number=order_number, vendor=request.user.vendor_profile)
        elif request.user.role == "delivery":
            order = Order.objects.get(order_number=order_number, delivery_person=request.user.delivery_profile)
        else:
            return Response({"error": "Sin permisos para ver este pedido"}, status=status.HTTP_403_FORBIDDEN)
        ...
        tracking_data = {
            ...
            "status_history": OrderStatusHistorySerializer(order.status_history.all(), many=True).data,
        }
