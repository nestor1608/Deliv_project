...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        category, _ = VendorCategory.objects.get_or_create(
            name="Restaurante", defaults={"description": "Restaurantes y pizzerías"}
        )

        # Crear perfiles
        customer, _ = Customer.objects.get_or_create(user=customer_user, defaults={"loyalty_points": 500})
        ...
        for product_data in products_data:
            product, created = Product.objects.get_or_create(
                vendor=vendor, name=product_data["name"], defaults=product_data
            )
        ...
        if created:
            ...
            OrderItem.objects.get_or_create(
                ...
                defaults={"quantity": 1, "unit_price": products[0].price, "total_price": products[0].price},
            )

            OrderItem.objects.get_or_create(
                order=order,
                product=products[1],  # Pizza Pepperoni
                defaults={"quantity": 1, "unit_price": products[1].price, "total_price": products[1].price},
            )
