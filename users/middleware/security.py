        ...
        for endpoint, limits in self.rate_limits.items():
            if path.startswith(endpoint):
                ...
                if current_requests >= limits["requests"]:
                    ...
                    return JsonResponse({"detail": "Demasiadas solicitudes. Intenta más tarde."}, status=429)
        ...
        try:
            ...
            for pattern in suspicious_patterns:
                if re.search(pattern, request.path, re.IGNORECASE):
                    ...
                    return JsonResponse({"detail": "Solicitud no válida"}, status=400)
            ...
            for key, value in request.GET.items():
                for pattern in suspicious_patterns:
                    if re.search(pattern, str(value), re.IGNORECASE):
                        ...
                        return JsonResponse({"detail": "Parámetros no válidos"}, status=400)
                ...
                try:
                    ...
                    for pattern in suspicious_patterns:
                        if re.search(pattern, body_str, re.IGNORECASE):
                            ...
                            return JsonResponse({"detail": "Contenido no válido"}, status=400)
