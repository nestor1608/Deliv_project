        ...
        try:
            with transaction.atomic():
                ...
                if User.objects.filter(username__iexact=username).exists():
                    return Response(
                        {"username": ["Este nombre de usuario ya está en uso."]}, status=status.HTTP_400_BAD_REQUEST
                    )
...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: OpenApiResponse(
                description="Error de autenticación - credenciales inválidas, cuenta inactiva o rol incorrecto"
            ),
        },
        tags=["auth"],
        examples=[
        ...
        try:
            ...
            for field in restricted_fields:
                if field in request.data:
                    return Response(
                        {"detail": f"No se puede modificar el campo {field}"}, status=status.HTTP_400_BAD_REQUEST
                    )
            ...
            return Response({"detail": "Error actualizando perfil"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def logout_view(request):
    ...
    try:
        ...
        return Response({"detail": "Error cerrando sesión"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                if not user.check_password(old_password):
                    ...
                    return Response(
                        {"old_password": ["Contraseña actual incorrecta."]}, status=status.HTTP_400_BAD_REQUEST
                    )
...
class PasswordResetRequestView(generics.GenericAPIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                description="Mensaje genérico indicando que si el email existe, se enviará el enlace de recuperación"
            ),
            429: OpenApiResponse(
                description="Demasiadas solicitudes - ya se envió un enlace recientemente. Espera 5 minutos."
            ),
        },
        tags=["auth"],
        examples=[
