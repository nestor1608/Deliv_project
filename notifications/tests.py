...
class NotificationViewSetTests(APITestCase):
    ...
    def test_mark_as_read_success(self):
        ...
        notification = NotificationFactory(user=self.user, notification_type=self.notification_type, status="sent")
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f"/api/notifications/notifications/{notification.id}/mark_as_read/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_mark_as_read_already_read_does_not_change_read_at(self):
        ...
        notification = NotificationFactory(
            user=self.user, notification_type=self.notification_type, status="read", read_at=timezone.now()
        )
    ...
    def test_mark_all_read_success(self):
        ...
        NotificationFactory.create_batch(3, user=self.user, notification_type=self.notification_type, status="sent")
        NotificationFactory(user=self.user, notification_type=self.notification_type, status="read")
        self.client.force_authenticate(user=self.user)
        response = self.client.patch("/api/notifications/notifications/mark_all_read/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Notification.objects.filter(user=self.user, status="read").count(), 4)
    ...
    def test_unread_count_correct(self):
        ...
        NotificationFactory.create_batch(5, user=self.user, notification_type=self.notification_type, status="sent")
        NotificationFactory(user=self.user, notification_type=self.notification_type, status="read")
        self.client.force_authenticate(user=self.user)
    ...
    def test_clear_old_deletes_old_read_notifications(self):
        ...
        old = NotificationFactory(user=self.user, notification_type=self.notification_type, status="read")
        Notification.objects.filter(id=old.id).update(read_at=cutoff)

        recent = NotificationFactory(user=self.user, notification_type=self.notification_type, status="read")
        self.client.force_authenticate(user=self.user)
    ...
    def test_clear_old_does_not_delete_unread(self):
        ...
        cutoff = timezone.now() - timedelta(days=31)
        unread = NotificationFactory(user=self.user, notification_type=self.notification_type, status="sent")
...
class UserDeviceViewSetTests(APITestCase):
    ...
    def test_toggle_active_toggles(self):
        ...
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f"/api/notifications/devices/{device.id}/toggle_active/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_test_notification_success(self, mock_fcm_service):
        ...
        device = UserDeviceFactory(user=self.user)
        self.client.force_authenticate(user=self.user)
        response = self.client.post(f"/api/notifications/devices/{device.id}/test_notification/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
