...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def mark_as_read(self, request, pk=None):
        ...
        return Response(
            {"message": "Notificación marcada como leída", "notification": NotificationSerializer(notification).data}
        )
    ...
    def mark_all_read(self, request):
        ...
        unread_notifications = Notification.objects.filter(
            user=request.user, status__in=["pending", "sent", "delivered"]
        )

        count = unread_notifications.update(status="read", read_at=timezone.now())
    ...
    def unread_count(self, request):
        ...
        count = Notification.objects.filter(user=request.user, status__in=["pending", "sent", "delivered"]).count()
    ...
    def clear_old(self, request):
        ...
        deleted_count, _ = Notification.objects.filter(
            user=request.user, status="read", read_at__lt=cutoff_date
        ).delete()

        return Response({"message": f"{deleted_count} notificaciones antiguas eliminadas"})
...
class UserDeviceViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if device_id:
            UserDevice.objects.filter(user=self.request.user, device_id=device_id).update(is_active=False)
    ...
    def test_notification(self, request, pk=None):
        ...
        return Response({"message": "Notificación de prueba enviada", "success": success})
...
def send_bulk_notification(request):
    ...
    if not users.exists():
        return Response(
            {"error": "No se encontraron usuarios para enviar la notificación"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def send_custom_notification(request):
    ...
    fcm_service = FCMService()
    success = fcm_service.send_notification(request.user, title, body, data)

    return Response({"message": "Notificación enviada", "success": success, "title": title, "body": body})
...
def notification_stats(request):
    ...
    stats = {
        ...
        "this_week": user_notifications.filter(created_at__gte=timezone.now() - timezone.timedelta(days=7)).count(),
    }

    # Notificaciones por tipo
    notification_types = (
        user_notifications.values("notification_type__name").annotate(count=models.Count("id")).order_by("-count")[:5]
    )
...
def delete_all_notifications(request):
    ...
    return Response({"message": f"{deleted_count} notificaciones eliminadas"})
...
def register_device_token(request):
    ...
    if not fcm_token or not platform:
        return Response({"error": "Se requieren fcm_token y platform"}, status=status.HTTP_400_BAD_REQUEST)

    # Desactivar dispositivos anteriores del mismo usuario con el mismo token
    UserDevice.objects.filter(user=request.user, fcm_token=fcm_token).update(is_active=False)
    ...
    return Response({"message": "Token registrado correctamente", "device": UserDeviceSerializer(device).data})
